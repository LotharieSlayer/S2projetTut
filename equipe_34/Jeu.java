...
public class Jeu
{
    ...
    public boolean actionReconnue(char action)
    {
      if ( action == 'C' || action == 'c' || action == 'L' || action == 'l' ) return true;
      else return false;
    }
    
    public boolean SystemeSolaireReconnue(char systemeSolaire)
    {
      if (systemeSolaire == 'T' || systemeSolaire == 'U' || systemeSolaire == 'M' || systemeSolaire == 'Q') return true;
      else return false;
    }
    ...
    public Jeu()
    {
        ...
        while(this.joueurA.getNbAnneaux() < 8 && this.joueurB.getNbAnneaux() < 8)
        {
            ...
            while(this.debutTour == false)
            {
                ...
                while (cpt < 4)
                {
                    System.out.print(this.galaxie.getSystemesSolaires().get(cpt).getNom()
                                   + this.galaxie.getSystemesSolaires().get(cpt).getNbPlanete()
                                   + this.galaxie.getSystemesSolaires().get(cpt).getNBPlaneteJoueurA()
                                   + this.galaxie.getSystemesSolaires().get(cpt).getNBPlaneteJoueurB()
                                   + (this.galaxie.getSystemesSolaires().get(cpt).getNBPlaneteJoueurA() - this.galaxie.getSystemesSolaires().get(cpt).getNBPlaneteJoueurB() ) );
                    cpt ++;
                }

                System.out.println("Voulez-vous conquérir ou libérer une planète Seigneur " + nomSeigneur + " ? (C/l)");
                action = Clavier.lire_char();
                while (actionReconnue(action) = false)
                {
                  System.out.print("Il me semble qu'il y ait une erreure dans votre action. Pouvez-vous répéter ? ");
                  action = Clavier.lire_char();
                }

                System.out.print("Donnez la première lettre du Système Solaire où effectué l'action :");
                systemeSolaire = Clavier.lire_char();
                while (SystemeSolaireReconnue(systemeSolaire) = false)
                {
                  System.out.print("Je n'ai pas bien compris. Où ça? ");
                  action = Clavier.lire_char();
                }
